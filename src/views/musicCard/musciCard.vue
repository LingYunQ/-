...
export default {
  ...
  watch: {
    "$store.state.isMusicDetailCardShow"(isMusicDetailCardShow) {
      ...
      //删除定时器 避免用户在关闭卡片的1s内又打开卡片导致展示内容被删除
      clearTimeout(timer);
      this.cleanCard = false;
      if (
        isMusicDetailCardShow &&!this.comment.comments
      ) {
        // 传入歌曲id请求歌词
  ...
  methods: {
   ...
   async commentPageChange(page) {
      ...
      // 请求评论
     await this.getMusicComment(this.$store.state.musicId);
    ...
    closeCard() {
     ...
     // 返回评论头部
    scrollToComment() {
      let listInfo = document.querySelector(".musicDetailCardContainer");
      listInfo.scrollTo({
        behavior: "smooth",
        top: 430,
      });
...
.musicDetailCard {
  ...
  background-image: linear-gradient(to bottom, #dfe6e9,#fff);
}
